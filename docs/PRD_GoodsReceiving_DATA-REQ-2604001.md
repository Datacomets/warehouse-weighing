...
### 26 พฤษภาคม 2569 (v2.6) — Admin/Manager Direct Edit on Completed

**เปลี่ยน Flow แก้ไขเอกสารหลังนำเข้า SAP**
- **ลบ "Reopen completed"** (Flow v2.5) — เดิม Admin/Admin SAP กด "แก้ไขเอกสาร" → DB reset status เป็น in_progress + clear closed_at → เอกสารกลับไปแสดงในงานของ operator → operator แก้ + ส่งใหม่
- **ใหม่: แก้ไขโดยตรง (in-place edit)** — Admin / Admin SAP / **Manager** กดปุ่ม "แก้ไขข้อมูล" บน `/admin/[id]` → Link ไป `/doc/[id]/header` ทันที (ไม่เปลี่ยน status, ไม่เคลียร์ timestamp) → แก้ field ใด ๆ → auto-save → audit_log `edit_header` บันทึก actor
- **เอกสารไม่กลับไปแสดงในงานของ operator** เพราะ status ยังเป็น `completed`
- **เพิ่มสิทธิ์ Manager:** ก่อนหน้านี้ admin + admin_sap → ตอนนี้รวม manager ด้วย
- **Banner เตือนบนหน้าแก้:** หน้า `/doc/[id]/*` แสดง banner สีส้ม "⚠ เอกสารนี้นำเข้า SAP แล้ว — การแก้ไขจะไม่เปลี่ยนสถานะกลับ แต่ระบบจะบันทึก audit log ทุกครั้ง"

**Pending_SAP behavior — ไม่เปลี่ยน**
- ยังต้องใช้ปุ่ม **"ปลดล็อก"** (Unlock) → operator แก้ → ส่งใหม่ (เหมือนเดิม)
- Admin/Manager **ไม่สามารถ** แก้ pending_sap doc โดยตรง — ต้อง unlock ก่อน

**Helper / API changes**
- เพิ่ม `canEditDocumentData(role, status)` ใน `src/lib/workflow.ts`:
  - admin/admin_sap/manager → in_progress OR completed
  - operator/qc → in_progress only (พฤติกรรมเดิม)
- ลบ `canReopen()`, `canReopenCompleted()`, `reopenCompletedDocument()` (dead code)
- ลบ component `ReopenButton.tsx`
- audit_log action `reopen_completed` ไม่ถูกบันทึกอีก (เก็บข้อมูลเก่าไว้ที่ DB)
...
[กำลังดำเนินการ] ──ส่งงาน──► [รอนำเข้า SAP] ──นำเข้า SAP──► [เสร็จสมบูรณ์]
       ▲                          │                              │
       │                          │                              │
       │           ปลดล็อก (admin) │              แก้ไขข้อมูล (admin/admin_sap/manager)
       └──────────────────────────┘                              │  ↻ status คงเดิม
                                                                 ▼
                                                          [เสร็จสมบูรณ์]
```

> **หมายเหตุ:** จาก v2.6 การแก้ไขเอกสารที่ "เสร็จสมบูรณ์" จะ **ไม่เปลี่ยน status** —
> Admin/Admin SAP/Manager แก้ field ได้โดยตรง โดย doc ยังคงเป็น `completed`
> และเอกสารจะไม่กลับไปแสดงในงานของ operator
...
- เมื่อ Status เป็น "เสร็จสมบูรณ์" — **Admin / Admin SAP / Manager** สามารถกด **"แก้ไขข้อมูล"** บน `/admin/[id]` → Link ไป `/doc/[id]/header` เพื่อแก้ไขข้อมูลโดยตรง **โดย status ไม่เปลี่ยน** (เอกสารยังคงเป็น completed, ไม่กลับไปแสดงในงานของ operator) — ทุกการแก้ไขถูกบันทึก audit_log ตามปกติ (`edit_header` เป็นต้น)
...
| ปุ่ม Action | Export PDF · ปลดล็อก (pending_sap, admin_sap+admin) · **แก้ไขข้อมูล** (completed → in-place edit, admin/admin_sap/manager) |
